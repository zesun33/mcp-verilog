#!/usr/bin/env bash
# verify.sh — 10-gate verification flow for mcp-verilog.
#
...
for arg in "$@"; do
  case "$arg" in
    ...
    -h|--help)
      cat <<'EOF'
verify.sh — mcp-verilog verification suite
...
run_gate_4() {
  echo "--- Gate 4: Fixture Integration (Podman Simulation & Lint) ---"
  npx tsx --test tests/integration.test.ts || fail 4 "Integration tests failed"
  pass 4 "Podman container integration passed on all fixtures"
}

run_gate_5() {
  echo "--- Gate 5: Protocol Contract (stdio JSON-RPC) ---"
  RESPONSE=$(echo '{"jsonrpc":"2.0","id":1,"method":"tools/list"}' | node dist/index.js)
  echo "${RESPONSE}" | grep -q "verilog_simulate" || fail 5 "Tools list does not contain verilog_simulate"
...
run_gate_6() {
  ...
  grep -q "verilog_simulate" README.md || fail 6 "README missing verilog_simulate docs"
...
case "${GATE}" in
  ...
  "")
    ...
    run_gate_6
    echo ""
    echo -e "\033[0;32m=== All Gates Cleared: mcp-verilog Verified ===\033[0m"
    ;;
  *)
